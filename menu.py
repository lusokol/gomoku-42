...
def main():
    ...
    while run:
        ...
        # necessaire uniquement lors d'un changement de taille de
        # screen mais se fait a tous les ticks pour le moment (à changer?)
        screen.blit(background_img, (0, 0))
        newH = 30 * config.HPC
        newW = newH / aspect_ratio
        logo = pygame.transform.scale(logo, (newW, newH))
        game_surface = pygame.Surface(
            (config.SCREEN_WIDTH // 3 * 2, config.SCREEN_HEIGHT),
            pygame.SRCALPHA,
        )
